...
function AdminLeaderboard() {
  ...
  const fetchLeaders = async () => {
    try {
      ...
      const response = await axios.get('https://knowledge-challenge-fullstack-pt8i.vercel.app/api/results/leaderboard', config);
  ...
  const handleDelete = async (resultId) => {
    if (window.confirm("هل أنت متأكد من حذف هذه النتيجة نهائياً؟")) {
      try {
        ...
        await axios.delete(`https://knowledge-challenge-fullstack-pt8i.vercel.app/api/results/${resultId}`, config);
